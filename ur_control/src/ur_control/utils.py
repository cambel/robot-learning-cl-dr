...
class PID:
    ...
    def update(self, error, dt=None):
        ...
        if self.dynamic_pid:
            # cm
            kp = np.abs([self.Kp[i]/(error[i]*100) if error[i] != 0.0 and error[i] < 0.01 else self.Kp[i] for i in range(6)])
            kp = np.clip(kp, self.Kp, self.Kp*self.max_gain_multiplier)
            # cm
            kd = np.abs([self.Kd[i]*(error[i]*100) if error[i] != 0.0 and error[i] < 0.01 else self.Kd[i] for i in range(6)])
